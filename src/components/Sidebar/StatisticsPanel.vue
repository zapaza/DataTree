<template>
  <div class="p-4 space-y-4">
    ...
    <div v-if="documentStore.parsedData" class="grid grid-cols-2 gap-2">
      <div class="bg-base p-3 rounded border border-light shadow-sm">
        <div class="text-[9px] text-light uppercase font-bold">{{ t('stats.nodes') }}</div>
        <div class="text-lg font-bold text-base">{{ stats.nodes }}</div>
      </div>
      <div class="bg-base p-3 rounded border border-light shadow-sm">
        <div class="text-[9px] text-light uppercase font-bold">{{ t('stats.size') }}</div>
    ...
    <div v-if="documentStore.parsedData" class="space-y-1.5 pt-2 border-t border-base">
      <div class="flex justify-between text-[10px] py-1 border-b border-light">
        <span class="text-light uppercase font-bold">{{ t('stats.format') }}</span>
      ...
      <div class="flex justify-between text-[10px] py-1">
        <span class="text-light uppercase font-bold">{{ t('stats.status') }}</span>
    ...
    <div v-else class="flex flex-col items-center justify-center p-6 text-center bg-secondary rounded-xl border border-dashed border-base">
      ...
      <p class="text-[10px] text-muted italic">{{ t('stats.enterData') }}</p>
    </div>
  </div>
</template>
