<template>
  <div v-if="enhancedErrors.length > 0" class="error-panel absolute bottom-0 left-0 right-0 z-20 max-h-48 overflow-y-auto animate-slide-up">
    <div class="bg-red-100 dark:bg-red-950 border-t border-red-300 dark:border-red-800 shadow-2xl">
      <div v-for="(error, index) in enhancedErrors" :key="index" class="p-3 border-b border-red-200 dark:border-red-900 last:border-0">
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-start gap-3">
            <div class="i-carbon-error-filled text-red-600 dark:text-red-500 text-lg mt-0.5 shrink-0" />
            <div class="space-y-1">
