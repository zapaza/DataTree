    ...
    <div class="flex-1 flex min-h-0 overflow-hidden relative">
        ...
        <div v-if="diffStore.diffResult" class="px-4 py-2 border-b border-base bg-secondary flex gap-4 items-center justify-between shrink-0">
          <div class="flex gap-4 items-center">
            <div class="flex items-center gap-1.5">
              ...
              <span class="text-[10px] font-bold uppercase text-muted">{{ t('compare.added') }}: {{ diffStore.diffResult.stats.added }}</span>
            </div>
            <div class="flex items-center gap-1.5">
              <div class="w-2 h-2 rounded-full bg-red-500" />
              <span class="text-[10px] font-bold uppercase text-muted">{{ t('compare.removed') }}: {{ diffStore.diffResult.stats.removed }}</span>
            </div>
            <div class="flex items-center gap-1.5">
              <div class="w-2 h-2 rounded-full bg-amber-500" />
              <span class="text-[10px] font-bold uppercase text-muted">{{ t('compare.modified') }}: {{ diffStore.diffResult.stats.modified }}</span>
            </div>
            <div class="hidden xl:flex items-center gap-1.5">
              <div class="w-2 h-2 rounded-full bg-red-500" />
              <span class="text-[10px] font-bold uppercase text-muted">{{ t('compare.breaking') }}: {{ diffStore.diffResult.riskSummary.breaking }}</span>
            </div>
            <div class="hidden xl:flex items-center gap-1.5">
              <div class="w-2 h-2 rounded-full bg-amber-500" />
              <span class="text-[10px] font-bold uppercase text-muted">{{ t('compare.warnings') }}: {{ diffStore.diffResult.riskSummary.warnings }}</span>
            </div>
          </div>
        ...
        <div class="flex-1 relative min-h-0">
          ...
          >
            ...
            <template #left-header>
              <div class="px-4 h-10 border-b border-light flex items-center bg-secondary justify-between">
                <div class="flex items-center gap-2">
                  <span class="text-xs font-semibold uppercase tracking-wider text-muted">{{ t('compare.sourceA') }}</span>
                ...
                <div class="flex gap-2">
                  <div
                    v-if="!diffStore.left.valid && diffStore.left.raw.trim()"
                    class="flex items-center gap-1 text-[10px] text-red-500 font-bold animate-pulse"
                  >
                    <div class="i-carbon-error text-xs" />
                    <span>{{ t('common.invalid') }} {{ diffStore.left.format.toUpperCase() }}</span>
                  </div>
                  <button
            ...
            <template #right-header>
              <div class="px-4 h-10 border-b border-light flex items-center bg-secondary justify-between">
                <div class="flex items-center gap-2">
                  <span class="text-xs font-semibold uppercase tracking-wider text-muted">{{ t('compare.sourceB') }}</span>
                ...
                <div class="flex gap-2">
                  <div
                    v-if="!diffStore.right.valid && diffStore.right.raw.trim()"
                    class="flex items-center gap-1 text-[10px] text-red-500 font-bold animate-pulse"
                  >
                    <div class="i-carbon-error text-xs" />
                    <span>{{ t('common.invalid') }} {{ diffStore.right.format.toUpperCase() }}</span>
                  </div>
                  <button
                    class="p-1 hover:bg-gray-200 dark:hover:bg-[#2d2d2d] rounded transition-colors text-muted"
                    :title="t('file.import')"
      ...
      <div class="w-80 flex flex-col bg-sidebar shrink-0 overflow-hidden border-l border-base">
        <div class="p-4 border-b border-base bg-secondary">
          <h2 class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('compare.summary') }}</h2>
        </div>
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          <DiffSummary :diff-result="diffStore.diffResult" />

          <div v-if="viewMode === 'editor'" class="p-4 border-t border-base">
            <h3 class="text-[10px] font-bold uppercase text-muted mb-3">{{ t('compare.changesList') }}</h3>
            ...
            <div class="space-y-1">
              <div
                ...
                :key="index"
                class="w-full text-left px-2 py-1.5 rounded text-[10px] font-mono hover:bg-gray-200 dark:hover:bg-[#2d2d2d] transition-colors flex items-center gap-2 cursor-pointer"
                ...
                @keydown.enter.prevent="handleSelectChange(index)"
                @keydown.space.prevent="handleSelectChange(index)"
              >
                <div
                ...
                <span class="truncate min-w-0 flex-1">{{ change.displayPath || change.path }}</span>
                <button
                  class="shrink-0 p-1 rounded hover:bg-white/70 dark:hover:bg-black/20 text-muted"
                  :title="t('compare.copyPath')"
                  @click.stop="copyChangePath(change)"
                >
                  <div class="i-carbon-copy text-xs" />
                </button>
                <button
                  class="shrink-0 p-1 rounded hover:bg-white/70 dark:hover:bg-black/20 text-muted"
                  :title="t('compare.copyNewValue')"
                  @click.stop="copyChangeValue(change)"
                >
                  <div class="i-carbon-code text-xs" />
                </button>
              </div>
              ...
              <button
                ...
                @click="loadMore"
              >
                {{ t('compare.loadMoreChanges', { count: filteredChangesAll.length - displayLimit }) }}
              </button>
            </div>
          </div>
        </div>
...
import type { TDiffNode, TDiffRisk } from '@/types/diff';
...
const riskClass = (risk?: TDiffRisk) => {
...
const riskLabel = (risk?: TDiffRisk) => {
  if (risk === 'breaking') return t('compare.breaking');
  if (risk === 'non-breaking') return t('compare.nonBreaking');
  if (risk === 'warning') return t('compare.warnings');
  return t('compare.neutral');
};

const stringifyChangeValue = (change: TDiffNode) => {
  const value = change.newValue !== undefined ? change.newValue : change.oldValue;
  return typeof value === 'object' && value !== null ? JSON.stringify(value, null, 2) : String(value ?? '');
};

const copyChangePath = (change: TDiffNode) => {
  copy(change.displayPath || change.path, t('compare.changePathCopied'));
};

const copyChangeValue = (change: TDiffNode) => {
  copy(stringifyChangeValue(change), t('compare.changeValueCopied'));
};
...
const onLeftFileChange = async (e: Event) => {
  ...
  if (file) {
    try {
      ...
      showToast(t('file.importedToSource', { name: file.name, source: t('compare.sourceA') }), 'success');
    } catch {
...
const onRightFileChange = async (e: Event) => {
  ...
  if (file) {
    try {
      ...
      showToast(t('file.importedToSource', { name: file.name, source: t('compare.sourceB') }), 'success');
    } catch {
      showToast(t('file.readFailed'), 'error');
    } finally {
      target.value = '';
